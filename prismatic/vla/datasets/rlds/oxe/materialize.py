...
def make_vqa_dataset_kwargs(
    ...
    """Generates config (kwargs) for given dataset from VQA."""
    dataset_kwargs = deepcopy(OXE_DATASET_CONFIGS[dataset_name])
    if dataset_kwargs["action_encoding"] not in [ActionEncoding.EEF_POS, ActionEncoding.EEF_R6]:
        raise ValueError(f"Cannot load `{dataset_name}`; only EEF_POS & EEF_R6 actions supported!")
    ...
    dataset_kwargs["ques_key"] = "question_text"
    dataset_kwargs["ans_key"] = "top_answer"
    dataset_kwargs["all_ans_key"] = "answers"
...
def get_oxe_dataset_kwargs_and_weights(
    ...
    per_dataset_kwargs, sampling_weights, per_dataset_types = [], [], []
    for d_name, d_weight in filtered_mixture_spec:
        try:
            d_type = OXE_DATASET_CONFIGS[d_name]["type"] if "type" in OXE_DATASET_CONFIGS[d_name] else "action"
            per_dataset_kwargs.append(
                make_vqa_dataset_kwargs(
                    d_name,
                    data_root_dir,
                )
                if d_type == "vqa" else
            ...
            per_dataset_types.append(d_type)

        except ValueError as e:
            overwatch.warning(f"Skipping `{d_name}` due to Error: {e}")

    return per_dataset_kwargs, sampling_weights, per_dataset_types
